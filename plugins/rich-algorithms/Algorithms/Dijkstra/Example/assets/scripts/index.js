// Code adapted from Lettier, 2014
...
function on_load( )
{
	...
	var STARTING_NODE_COLOR = "#549a24";
	var ENDING_NODE_COLOR   = "#cd201f";
	...
	function shortest_path_algorithm( parameters )
	{
		...
		if ( predecessor_node != -1 )
		{
			...
			shortest_path.push(current_circle);
		...
		let path_size = shortest_path.length - 1;

		if (shortest_path.length) {
			for (let i = 0; i < shortest_path.length; i++) {
				//console.log(shortest_path[i].text.string);
				path_string += shortest_path[i].text.string;
				if (path_size > i) {
					path_string += " --> ";
				}
			}
